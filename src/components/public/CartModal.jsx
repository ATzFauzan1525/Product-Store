...
export default function CartModal({
  ...
  const handleCheckout = () => {
    ...
    const zeroStockItems = selectedCartItems.filter(
    ...
    if (zeroStockItems.length > 0) {
      ...
      if (window.confirm(confirmMsg)) {
        ...
        const remaining = selectedCartItems.filter(
          (item) => Number(item.stock) > 0,
        );
        if (remaining.length === 0) {
          alert(
            "Semua produk yang dipilih stoknya kosong dan telah dihapus dari keranjang.",
          );
          return;
        }
